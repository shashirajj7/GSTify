...
AMOUNT_PATTERN = r"(?<!\d)\d{1,7}(?:[,\s]\d{3})*(?:\.\d{2})(?!\d)"
...
def parse_fields(text):
    ...
    for i, line in enumerate(lines):
        ...
        if "GST" in line_upper and "IN" not in line_upper and "EXCLUD" not in line_upper: # avoid GSTIN line and EXCLUDE lines
            val = extract_number_from_line(line_upper)
            if val:
    ...
    # Fallback: if total isn't explicitly found, take the largest amount found
    if total is None:
        all_amounts = [extract_number_from_line(l.upper()) for l in lines]
        valid_amounts = [a for a in all_amounts if a is not None]
        if valid_amounts:
            total = max(valid_amounts)
